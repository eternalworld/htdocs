		...
		$.ajax({
			...
			success: function(response) {
				...
				if(resSlave[1]!=""){
					...
					$("#slave").html('Slave：wait');
					$("#program").html('Program：wait');
					$("#delay").html('Delay：wait');
				}
			}
		});
						...
						$.ajax({
							...
							success: function(response) {
								...
								if(response[0]!="false"){
									...
									if(response.sort().toString() != resLight.sort().toString()){
										resLight = response;
										resArray();
										changeLightS(1);
									}
